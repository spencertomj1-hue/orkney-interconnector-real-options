...
# Splits the single 220MW NewLink into N stages, each priced fixed_per_stage + variable_permw*MW with a learning-curve discount on variable_permw for later stages; fixed_per_stage stays flat every stage (no economies of scale from splitting one contract into several).
# The phased-deployment design alternative [5] evaluates against a single fixed-capacity build for PV, applied here to transmission capacity.
...
# UNCALIBRATED: should be a later phase's capacity-independent mobilisation cost when reusing stage 1's route/consent, not a full new project's; no real precedent found (Table A3.3, SSEN's 2024 Orkney-Caithness contract, generic HVDC benchmarks -- the real project isn't staged).
# Set to ~1/5 of a hypothetical +30MW second link's fixed cost (110e6 * REBASE, [1] Table A3.3's linear fit, an upper bound since that prices a wholly separate circuit) -- swept rather than guessed harder, see the SWEEP below.
...
# variable £/MW before learning discount -- [1] Table A3.3's linear fit slope
...
class StagedLinkStage:
    def __init__(self, capacity_mw, fixed_per_stage, variable_permw, capex_mult=1.0):
        ...
        self.__OPEX_Rate = LINK_OPEX_RATE   # [3]/[4], same rate as NewLink
